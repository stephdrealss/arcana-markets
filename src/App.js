...
const ALL_MARKETS = [
  ...
  {id:81,title:"Will Argentina beat Cape Verde in the Round of 32?",cat:"Sports",yes:0.82,chg:0,vol:"0",ends:"Jul 3 2026",closeTime:"2026-07-03T18:00:00-04:00",resolutionSource:"Official FIFA World Cup 2026 result",betDefinition:"\"Beat\" means advancing by any means, including via penalty shootout.",voidCondition:"Void — full refund if the match is not played."},
  {id:82,title:"Will Australia beat Egypt in the Round of 32?",cat:"Sports",yes:0.40,chg:0,vol:"0",ends:"Jul 3 2026",closeTime:"2026-07-03T14:00:00-04:00",resolutionSource:"Official FIFA World Cup 2026 result",betDefinition:"\"Beat\" means advancing by any means, including via penalty shootout.",voidCondition:"Void — full refund if the match is not played."},
  {id:83,title:"Will Colombia beat Ghana in the Round of 32?",cat:"Sports",yes:0.63,chg:0,vol:"0",ends:"Jul 3 2026",closeTime:"2026-07-03T21:30:00-04:00",resolutionSource:"Official FIFA World Cup 2026 result",betDefinition:"\"Beat\" means advancing by any means, including via penalty shootout.",voidCondition:"Void — full refund if the match is not played."},
  {id:84,title:"Will the USA beat Belgium in the Round of 16?",cat:"Sports",yes:0.35,chg:0,vol:"0",ends:"Jul 6 2026",closeTime:"2026-07-06T12:00:00-04:00",resolutionSource:"Official FIFA World Cup 2026 result",betDefinition:"\"Beat\" means advancing by any means, including via penalty shootout.",voidCondition:"Void — full refund if the match is not played. Kickoff time assumed noon ET pending confirmation; close time to be adjusted to actual kickoff."},
  {id:85,title:"Will Mexico beat England in the Round of 16?",cat:"Sports",yes:0.30,chg:0,vol:"0",ends:"Jul 6 2026",closeTime:"2026-07-06T12:00:00-04:00",resolutionSource:"Official FIFA World Cup 2026 result",betDefinition:"\"Beat\" means advancing by any means, including via penalty shootout.",voidCondition:"Void — full refund if the match is not played. Kickoff time assumed noon ET pending confirmation; close time to be adjusted to actual kickoff.",hot:true},
  {id:86,title:"Will Argentina win the FIFA World Cup 2026?",cat:"Sports",yes:0.22,chg:0,vol:"0",ends:"Jul 19 2026",closeTime:"2026-07-19T15:00:00-04:00",resolutionSource:"Official FIFA World Cup 2026 result, resolves after the Jul 19 final.",voidCondition:"Void — full refund if the tournament is not completed as scheduled."},
  {id:87,title:"Will France reach the World Cup 2026 final?",cat:"Sports",yes:0.28,chg:0,vol:"0",ends:"Jul 19 2026",closeTime:"2026-07-19T15:00:00-04:00",resolutionSource:"Official FIFA World Cup 2026 result, resolves after the Jul 19 final.",voidCondition:"Void — full refund if the tournament is not completed as scheduled."},
  {id:88,title:"Will Messi win the World Cup 2026 Golden Boot?",cat:"Sports",yes:0.15,chg:0,vol:"0",ends:"Jul 19 2026",closeTime:"2026-07-19T15:00:00-04:00",resolutionSource:"Per the official FIFA Golden Boot award for the 2026 tournament, resolves after the Jul 19 final.",voidCondition:"Void — full refund if the tournament is not completed as scheduled."},
];
...
function TradeModal({m,initSide,onClose,t,account,usdcBalance,onPositionAdded,onActivityAdded,walletType,walletId}){
  ...
  const placeOrder=async()=>{
    ...
    try{
      ...
      if(!mc){throw new Error("market not found on-chain");}
      if(mc.endTime>0&&mc.endTime<nowSec){throw new Error("market has ended");}
      ...
      else if(msg.toLowerCase().includes("market not found"))setError("This market is not yet live on-chain. Please try again shortly.");
      else if(msg.toLowerCase().includes("cancelled"))setError("This market has been cancelled.");
      ...
      <nav style={{position:"sticky",top:0,zIndex:100,background:t.navBg,borderBottom:`1px solid ${t.border}`}}>
        <div style={{maxWidth:1380,margin:"0 auto",padding:"0 20px",display:"flex",alignItems:"center",gap:16,height:56}}>
          ...
          <div className="nav-right" style={{display:"flex",gap:8,alignItems:"center",flexShrink:0}}>
            {account&&<div className="usdc-badge" style={{display:"flex",alignItems:"center",gap:6}}>   <div style={{padding:"6px 12px",background:t.greenBg,border:`1px solid ${t.greenBorder}`,borderRadius:8,fontSize:12,color:t.green,fontFamily:"monospace",fontWeight:700}}>${usdcBalance} USDC</div>   <button onClick={()=>setDepositOpen(o=>!o)} style={{padding:"6px 12px",background:t.blueDim,border:`1px solid ${t.blueBorder}`,borderRadius:8,fontSize:12,color:t.blue,fontFamily:"monospace",fontWeight:700,cursor:"pointer"}}>+ Deposit</button>   <button onClick={()=>setBridgeOpen(o=>!o)} style={{padding:"6px 12px",background:t.blueDim,border:`1px solid ${t.blueBorder}`,borderRadius:8,fontSize:12,color:t.blue,fontFamily:"monospace",fontWeight:700,cursor:"pointer"}}>⇄ Bridge</button> </div>}
