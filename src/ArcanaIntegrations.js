            ...
            <div style={{ padding:"16px 24px 24px" }}>
              <div style={{ background:"linear-gradient(135deg,#1d4ed8 0%,#2563eb 50%,#1e40af 100%)", borderRadius:14, padding:"18px 20px", marginBottom:16, border:"1.5px solid rgba(255,255,255,0.1)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
                  <div style={{ width:36, height:36, borderRadius:10, background:"rgba(255,255,255,0.15)", display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden" }}><img src="https://i.ibb.co/236kWvmc/circle-logo.png" width="36" height="36" style={{borderRadius:10}} /></div>
                  <div>
...
export function BridgePanel({ t, account, walletType, walletId, arcAddress, userId }) {   const [open, setOpen] = useState(false);   const [srcChain, setSrcChain] = useState("Ethereum_Sepolia");   const [amount, setAmount] = useState("");   const [loading, setLoading] = useState(false);   const [status, setStatus] = useState(null);   const [statusMsg, setStatusMsg] = useState("");   const [txHash, setTxHash] = useState("");    const SUPPORTED_CHAINS = [     {id:"Ethereum_Sepolia",label:"Ethereum",icon:<img src="https://assets.coingecko.com/coins/images/279/small/ethereum.png" width="20" height="20" style={{borderRadius:"50%"}}/>},     {id:"Base_Sepolia",label:"Base",icon:<svg width="20" height="20" viewBox="0 0 32 32"><circle cx="16" cy="16" r="16" fill="#0052FF"/><path d="M16 6C10.477 6 6 10.477 6 16s4.477 10 10 10c5.254 0 9.564-4.055 9.966-9.2H15.2v-1.6h10.8C25.564 10.455 21.254 6 16 6z" fill="white"/></svg>},     {id:"Avalanche_Fuji",label:"Avalanche",icon:<img src="https://assets.coingecko.com/coins/images/12559/small/Avalanche_Circle_RedWhite_Trans.png" width="20" height="20" style={{borderRadius:"50%"}}/>},   ];    const bridge = async () => {     if (!account) { setStatus("error"); setStatusMsg("Connect your wallet first"); return; }     if (!amount || parseFloat(amount) < 0.01) { setStatus("error"); setStatusMsg("Minimum 0.01 USDC"); return; }     setLoading(true); setStatus(null); setTxHash("");     try {       const sourceChain = srcChain === "Ethereum_Sepolia" ? "ETH-SEPOLIA" : srcChain === "Base_Sepolia" ? "BASE-SEPOLIA" : "AVAX-FUJI";       const res = await fetch("/api/bridge-cctp", {         method: "POST",         headers: { "Content-Type": "application/json" },         body: JSON.stringify({ userId, arcWalletId: walletId, arcAddress, sourceChain, amount }),       });       const data = await res.json();       if (data.step === "fund_required") {         setStatus("error");         setStatusMsg(`Fund your ${srcChain.replace(/_/g," ")} wallet with ${amount} USDC at faucet.circle.com — Address: ${data.sourceAddress}`);       } else if (data.step === "pending_attestation" || data.step === "complete" || data.success) {         setTxHash(data.burnTxHash || "");         setStatus("success");         setStatusMsg(data.message || `${amount} USDC bridged to Arc Testnet!`);         setAmount("");       } else {         throw new Error(data.error || "Bridge failed");       }     } catch (e) {       setStatus("error");       setStatusMsg(e.message?.slice(0, 100) || "Bridge failed");     }     setLoading(false);   };    if (!open) {     return (       <div style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 20px", background:t.blueDim, border:`1px solid ${t.blueBorder}`, borderRadius:12, marginBottom:24, cursor:"pointer" }} onClick={() => setOpen(true)}>         <span style={{ fontSize:18 }}>⇄</span>         <div style={{ flex:1 }}>           <p style={{ fontSize:13, fontWeight:700, color:t.text, margin:0 }}>Bridge USDC to Arc</p>           <p style={{ fontSize:11, color:t.textMuted, fontFamily:"monospace", margin:"2px 0 0" }}>Powered by Circle CCTP · Real onchain bridge</p>         </div>         <span style={{ fontSize:12, color:t.blue, fontFamily:"monospace" }}>Open →</span>       </div>     );   }    return (     <div style={{ background:t.surface, border:`1.5px solid ${t.border}`, borderRadius:14, padding:"20px 24px", marginBottom:24 }}>       <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>         <div>           <p style={{ fontSize:15, fontWeight:800, color:t.text, margin:0 }}>⇄ Bridge USDC to Arc</p>           <p style={{ fontSize:11, color:t.textMuted, fontFamily:"monospace", margin:"3px 0 0" }}>Powered by Circle CCTP · Real onchain bridge</p>         </div>         <button onClick={() => setOpen(false)} style={{ background:"none", border:"none", color:t.textMuted, cursor:"pointer", fontSize:18 }}>✕</button>       </div>       <div style={{ marginBottom:14 }}>         <label style={{ fontSize:11, color:t.textMuted, fontFamily:"monospace", letterSpacing:1, display:"block", marginBottom:6 }}>FROM CHAIN</label>         <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>           {SUPPORTED_CHAINS.map(c => (             <button key={c.id} onClick={() => setSrcChain(c.id)}               style={{ padding:"6px 12px", background:srcChain===c.id?t.blue:t.bg, border:`1px solid ${srcChain===c.id?t.blue:t.border}`, borderRadius:8, color:srcChain===c.id?"#fff":t.textMuted, fontSize:12, fontWeight:600, cursor:"pointer", display:"flex", alignItems:"center", gap:5 }}>               <span>{c.icon}</span> {c.label}             </button>           ))}         </div>       </div>       <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14, padding:"8px 12px", background:t.bg, borderRadius:8, border:`1px solid ${t.border}` }}>         <span style={{ fontSize:11, color:t.textMuted, fontFamily:"monospace" }}>TO</span>         <span style={{ fontSize:13, fontWeight:700, color:t.blue, fontFamily:"monospace" }}>◈ Arc Testnet</span>         <span style={{ fontSize:10, color:t.textMuted, fontFamily:"monospace", marginLeft:"auto" }}>USDC · Chain 0x4cef52</span>       </div>       <div style={{ marginBottom:14 }}>         <label style={{ fontSize:11, color:t.textMuted, fontFamily:"monospace", letterSpacing:1, display:"block", marginBottom:6 }}>AMOUNT</label>         <div style={{ display:"flex", alignItems:"center", background:t.bg, border:`1.5px solid ${t.border}`, borderRadius:10 }}>           <span style={{ padding:"12px", color:t.textMuted, fontFamily:"monospace" }}>$</span>           <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00"             style={{ flex:1, background:"none", border:"none", outline:"none", color:t.text, fontSize:16, fontFamily:"monospace", fontWeight:700, padding:"12px 0" }} />           <span style={{ padding:"12px 14px", color:t.textMuted, fontFamily:"monospace", fontSize:12 }}>USDC</span>         </div>         <div style={{ display:"flex", gap:6, marginTop:8 }}>           {["1","10","50","100"].map(v => (             <button key={v} onClick={() => setAmount(v)}               style={{ flex:1, padding:"5px 0", background:amount===v?t.blue:t.bg, border:`1px solid ${amount===v?t.blue:t.border}`, borderRadius:6, color:amount===v?"#fff":t.textMuted, fontSize:11, cursor:"pointer", fontFamily:"monospace" }}>               ${v}             </button>           ))}         </div>       </div>       {status==="success" && (         <div style={{ padding:"10px 14px", background:t.greenBg, border:`1px solid ${t.greenBorder}`, borderRadius:8, marginBottom:12 }}>           <p style={{ fontSize:12, color:t.green, fontFamily:"monospace", margin:0 }}>✓ {statusMsg}</p>           {txHash && <a href={`https://testnet.arcscan.app/tx/${txHash}`} target="_blank" rel="noreferrer" style={{ fontSize:11, color:t.blue, fontFamily:"monospace", textDecoration:"none", display:"block", marginTop:4 }}>↗ View on ArcScan</a>}         </div>       )}       {status==="error" && (         <div style={{ padding:"10px 14px", background:t.redBg, border:`1px solid ${t.redBorder}`, borderRadius:8, marginBottom:12 }}>           <p style={{ fontSize:12, color:t.red, fontFamily:"monospace", margin:0 }}>✕ {statusMsg}</p>         </div>       )}       {loading && (         <div style={{ padding:"10px 14px", background:t.blueDim, border:`1px solid ${t.blueBorder}`, borderRadius:8, marginBottom:12 }}>           <p style={{ fontSize:12, color:t.blue, fontFamily:"monospace", margin:0 }}>⏳ Bridging via Circle CCTP...</p>         </div>       )}       <button onClick={bridge} disabled={loading||!amount}         style={{ width:"100%", padding:"13px", background:loading?t.blueDim:t.blue, color:loading?t.blue:"#fff", border:`1.5px solid ${t.blue}`, borderRadius:10, fontWeight:800, fontSize:14, cursor:loading||!amount?"not-allowed":"pointer", fontFamily:"monospace", letterSpacing:0.5 }}>         {loading ? "⏳ BRIDGING VIA CCTP..." : `BRIDGE ${amount||"0"} USDC → ARC`}       </button>     </div>   ); }
  const [open, setOpen]           = useState(false);
  ...
  const SUPPORTED_CHAINS = [
    ...
    {id:"Arbitrum_Sepolia",label:"Arbitrum",icon:<img src="https://s2.coinmarketcap.com/static/img/coins/64x64/11841.png" width="20" height="20" style={{borderRadius:"50%"}}/>},
